...
from src.openadp.auth.keys import generate_keypair, private_key_to_jwk


class TestSecurityCriticalJWTValidation:
    """Test security-critical JWT validation edge cases that could enable attacks."""
    ...
    def test_malformed_der_signature_attack_sequence_tag(self):
        ...
        with patch('cryptography.hazmat.primitives.asymmetric.ec.EllipticCurvePrivateKey.sign') as mock_sign:
            ...
            with pytest.raises(ValueError, match="Invalid DER signature format"):
                make_dpop_header("POST", "https://example.com/token", private_key)
    ...
    def test_malformed_der_signature_attack_r_integer(self):
        ...
        with patch('cryptography.hazmat.primitives.asymmetric.ec.EllipticCurvePrivateKey.sign') as mock_sign:
            ...
            with pytest.raises(ValueError, match="Invalid DER signature format - expected INTEGER for r"):
                make_dpop_header("POST", "https://example.com/token", private_key)
    ...
    def test_malformed_der_signature_attack_s_integer(self):
        ...
        with patch('cryptography.hazmat.primitives.asymmetric.ec.EllipticCurvePrivateKey.sign') as mock_sign:
            ...
            with pytest.raises(ValueError, match="Invalid DER signature format - expected INTEGER for s"):
                make_dpop_header("POST", "https://example.com/token", private_key)
    ...
    def test_signature_coordinate_padding_attack(self):
        ...
        with patch('cryptography.hazmat.primitives.asymmetric.ec.EllipticCurvePrivateKey.sign') as mock_sign:
            ...
            assert len(dpop_header.split('.')) == 3  # Valid JWT format
    ...
    def test_missing_required_claims_attack_jti(self):
        ...
        with pytest.raises(ValueError, match="Missing required claim: jti"):
    ...
    def test_missing_required_claims_attack_htm(self):
        ...
        with pytest.raises(ValueError, match="Missing required claim: htm"):
            validate_dpop_claims(malicious_jwt, "POST", "https://example.com/token")
    ...
    def test_missing_required_claims_attack_htu(self):
        ...
        with pytest.raises(ValueError, match="Missing required claim: htu"):
            validate_dpop_claims(malicious_jwt, "POST", "https://example.com/token")
    ...
    def test_missing_required_claims_attack_iat(self):
        ...
        with pytest.raises(ValueError, match="Missing required claim: iat"):
            validate_dpop_claims(malicious_jwt, "POST", "https://example.com/token")
    ...
    def test_handshake_signature_verification_success_path(self):
        ...
        assert result is True  # Tests line 345


class TestAttackVectorMitigation:
    ...
    def test_url_confusion_attack_prevention(self):
        ...
        with pytest.raises(ValueError, match="HTTP URI mismatch"):
