...
func RecoverEncryptionKey(filename, password, userID string, serverURLs []string, threshold int, authCodes *AuthCodes) *RecoverEncryptionKeyResult {
	...
	clients := make([]*client.EncryptedOpenADPClient, 0, len(serverURLs))
	liveServerURLs := make([]string, 0, len(serverURLs))

	for _, serverURL := range serverURLs {
		client := client.NewEncryptedOpenADPClient(serverURL, nil)
		if err := client.Ping(); err == nil {
			clients = append(clients, client)
...
// This version accepts ServerInfo structs with public keys and uses encrypted clients when available.
// It performs the same recovery process as RecoverEncryptionKey but with proper Noise-NK encryption.
