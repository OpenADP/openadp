...
export class NoiseNK {
    ...
    async encrypt(plaintext) {
        ...
        const nonce = new Uint8Array(12);
        const view = new DataView(nonce.buffer);
        view.setUint32(4, Math.floor(this.n / 0x100000000), false);
        view.setUint32(8, this.n & 0xffffffff, false);
    ...
    async readMessageB(message) {
        ...
        this.k = keys.k1; // Use k1 for initiator sending
        this.n = 0;
    ...
    async writeMessageB(payload = new Uint8Array()) {
        ...
        this.handshakeComplete = true;
        const keys = this._split();
        this.k = keys.k2; // Use k2 for responder sending
        this.n = 0;
