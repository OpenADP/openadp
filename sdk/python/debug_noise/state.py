...
class SymmetricState(object):
    ...
    def initialize_symmetric(cls, noise_protocol: 'NoiseProtocol') -> 'SymmetricState':
        ...
        print(f"🔍 [PYTHON DEBUG] Initializing symmetric state")
        print(f"🔍 [PYTHON DEBUG] Protocol name: {noise_protocol.name}")
        print(f"🔍 [PYTHON DEBUG] Protocol name hex: {noise_protocol.name.hex()}")
        print(f"🔍 [PYTHON DEBUG] Protocol name length: {len(noise_protocol.name)}")
        print(f"🔍 [PYTHON DEBUG] Hash length: {noise_protocol.hash_fn.hashlen}")
        ...
        if len(noise_protocol.name) <= noise_protocol.hash_fn.hashlen:
            ...
            print(f"🔍 [PYTHON DEBUG] Protocol name <= HASHLEN, using padded name")
        else:
            instance.h = noise_protocol.hash_fn.hash(noise_protocol.name)
            print(f"🔍 [PYTHON DEBUG] Protocol name > HASHLEN, using hash")
            
        print(f"🔍 [PYTHON DEBUG] Initial h: {instance.h.hex()}")

        # Sets ck = h.
        instance.ck = instance.h
        print(f"🔍 [PYTHON DEBUG] Initial ck: {instance.ck.hex()}")
    ...
    def mix_hash(self, data: bytes):
        ...
        """
        print(f"🔍 [PYTHON DEBUG] mix_hash called")
        print(f"🔍 [PYTHON DEBUG] Current h: {self.h.hex()}")
        print(f"🔍 [PYTHON DEBUG] Mixing data: {data.hex()}")
        combined = self.h + data
        print(f"🔍 [PYTHON DEBUG] Combined (h + data): {combined.hex()}")
        self.h = self.noise_protocol.hash_fn.hash(self.h + data)
        print(f"🔍 [PYTHON DEBUG] New h: {self.h.hex()}")
        print(f"🔍 [PYTHON DEBUG] ---")
